...
def get_sales_data():
    ...
    by commas. The loop will repeatedly request data, until is valid.
    """
    while True:
...
def validate_data(values):
    ...
    try:
        ...
        if len(values) != 6:
            raise ValueError(
                f"Ecactly 6 values required, your provided {len(values)}"
            )
...
def calculate_surplus_data(sales_row):
    ...
    - Negative surplu indictes extra made when stock was sold out.
    """
...
def get_stock_values(data):
    """
    Print out the calculated stock number for each sandwich type.
    """
